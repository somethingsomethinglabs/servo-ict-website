...
```mermaid
flowchart LR
    A[Customers find the business] --> B[Website explains services and next steps]
    B --> C[Call or send an enquiry]
    C --> D[Owner receives and answers it]
    B --> E[Use an existing booking service]
    F[Business owns its website and email accounts] --> B
    F --> D
    G[Servo sets up access and explains routine tasks] --> F
```
