...
<style>
 ...
 .filters button { position:relative; min-height:48px; padding:.6rem 0; border:0; background:transparent; color:var(--muted); font:inherit; font-size:1.08rem; font-weight:700; cursor:pointer; }
 .filters button[aria-pressed='true'] { color:var(--ink); }
 .featured { display:grid; grid-template-columns:1.05fr 1fr; border:1px solid var(--seam); border-radius:10px; overflow:hidden; background-color:var(--plastic); box-shadow:var(--panel-shadow); }
 .featured > div { padding:clamp(1.75rem,3.5vw,3.25rem); }
 .featured img { display:block; width:100%; height:100%; object-fit:cover; }
 .topic { margin:0 0 1rem; color:var(--ink); font-size:.76rem; font-weight:700; letter-spacing:.1em; text-transform:uppercase; }
 .featured .topic { color:var(--ink); }
 h2,h3 { margin:0; font-weight:700; letter-spacing:-.045em; line-height:1.12; }
 ...
 .featured .button { margin-top:2rem; }
 ...
 .thumbnail img { display:block; width:100%; height:auto; aspect-ratio:1.7; object-fit:cover; border:1px solid var(--seam); }
