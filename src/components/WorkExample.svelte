...
<style>
	.work-example { display: grid; grid-template-columns: minmax(0, .85fr) minmax(0, 1.15fr); align-items: center; gap: clamp(2rem, 5vw, 5rem); width: 100%; margin-inline: auto; padding: clamp(2rem, 4vw, 3.5rem); background-color:var(--plastic); color: var(--ink); }
	.work-example.compact { grid-template-columns: minmax(0, .9fr) minmax(0, 1.15fr); gap: 3rem; padding: 0; background: none; }
	.example-kicker { margin: 0 0 1rem; font-size: .8rem; font-weight:700; letter-spacing: .12em; text-transform: uppercase; }
	h2 { max-width: 15ch; margin: 0; font-size: clamp(2.2rem, 3.9vw, 3.8rem); font-weight:700; line-height: 1.04; letter-spacing:-.04em; }
	.example-intro { max-width: 31rem; margin: 1.3rem 0; font-size: 1.2rem; line-height: 1.6; }
	details { margin: 1.5rem 0; border-block: 1px solid var(--line); }
	summary { padding-block: 1rem; cursor: pointer; font-size: .9rem; font-weight:700; }
	dl { margin: 0 0 1rem; }
	dl > div { padding-block: .65rem; }
	dt { font-weight:700; font-size: .85rem; }
	...
	.compact figcaption strong { display: block; font-size: 1.4rem; font-weight:700; line-height: 1.15; letter-spacing: -.04em; }
